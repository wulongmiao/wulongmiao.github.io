...
export NODE_OPTIONS="--max-old-space-size=4096" node最大内存配置
which node 查询当前命令执行路径
env export printenv 查看环境变量
pkill pname 按进程名杀死进程
kill -9 pid 按进程名pid强制杀死进程
echo PATH=$PATH:/usr/local/node/bin >> /etc/profile 所有用户追加环境变量
source ~/.bashrc 重新加载当前用户配置文件
export PATH=$PATH:/path/to/new/dir 当前会话追加环境变量
...
registry.k8s.io/foo/bar:latest
docker.elastic.co/foo/bar:latest
Container Registry	nvcr.io/foo/bar:latest
mcr.microsoft.com/foo/bar:latest


docker.m.ixdev.cn/library/nginx:latest
docker.m.ixdev.cn/grafana/grafana:latest
docker.m.ixdev.cn/foo/bar:latest
docker.m.ixdev.cn/foo/bar:latest
ghcr.m.ixdev.cn/foo/bar:latest
quay.m.ixdev.cn/foo/bar:latest
r8s.m.ixdev.cn/foo/bar:latest
dlastic.m.ixdev.cn/foo/bar:latest
Cvcr.m.ixdev.cn/foo/bar:latest
mcr.m.ixdev.cn/foo/bar:latest


m.ixdev.cn/docker.io/library/nginx:latest
m.ixdev.cn/docker.io/grafana/grafana:latest
m.ixdev.cn/registry-1.docker.io/foo/bar:latest
m.ixdev.cn/docker.io/foo/bar:latest
m.ixdev.cn/ghcr.io/foo/bar:latest
m.ixdev.cn/quay.io/foo/bar:latest
m.ixdev.cn/registry.k8s.io/foo/bar:latest
m.ixdev.cn/docker.elastic.co/foo/bar:latest
m.ixdev.cn/nvcr.io/foo/bar:latest
m.ixdev.cn/mcr.microsoft.com/foo/bar:latest
