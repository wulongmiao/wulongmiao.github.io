...
http {
  ...
  server {
      # location = 绝对匹配 location ^~ 高优先级前缀匹配   location ~ 正则匹配  location ~* 不区分大小写正则匹配
      # 代理地址+斜杠 【不会拼接上location的路径】  代理地址不加斜杠 【会拼上location的地址】
...
docker build -t aaa:ccc . // 默认制作镜像文件Dockerfile
...
docker-compose命令
```
docker-compose [command] --help 命令提示
docker compose up -d -f ttt-compose.yml -p my_project // 基于docker-compose.yml启动管理的容器
...
up 自动完成包括构建镜像、创建服务、启动服务并关闭关联服务相关容器的 一些列操作
version 打印版本信息
```
