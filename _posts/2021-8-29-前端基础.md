<!--
 * @Author: wulongmiao
 * @Date: 2021-12-25 09:46:37
 * @LastEditors: wulongmiao
 * @LastEditTime: 2022-01-14 11:36:05
 * @Description: 
-->
---
layout: post
...
<div><a href="#designMode" target="_self">7. 设计模式</a></div>
...
实现：整体广度优先算法,同层进行比较,同层遍历按照深度优先算法比较。对比当前同层的虚拟节点是否为同一种类型的标签patch,不同直接替换replace

相同patchVnode：newNode有子节点,old没有,将new添加到dom；old有,new无,删除真实dom子节点；都有文本节点,dom文本节点设置成new的文本节点；都有子节点,深度优先算法,双指针比对子节点,头节点相同,向后移,尾节点相同,向前移,不同则查找旧节点是否存在新节点的key,存在插入旧节点前,不存在新增,头指针大于尾指针裁剪old剩余节点


vue3 diff算法: 前置后置预处理,保存new剩余未处理节点,生成对应长度数组初始化-1,使用hash表,根据new保存old节点下标,找出数组最长递增子序列,使用最长递增子序列优化了diff对比流程
...
类似vue2对比流程
```
vue3对事件进行缓存,静态标记,静态提升


react diff算法

递增法,对比新列表中的节点,在原本位置列表中是否是的递增吗,判断节点是否需要移动
