...
```
减少http请求(防抖节流;合并http请求,不能充分利用缓存,更新频繁不适合,会频繁更新缓存标识)
gzip,Brotli压缩资源(无损压缩,服务器增加压力)
充分利用缓存,强缓存,协商缓存,减轻压力
减少域名数,cdn加速静态资源
代码优化:减少对doM的添加,删除操作,减少浏览器重绘回流;多情况判断用hash表代替ifelse;利用垃圾回收,频繁回收的对象用对象池管理(其他自动回收的高级语言);reduce;事件代理
单页应用首屏优化:预渲染(骨架屏)

webpack:
        构建过程:  sourcemap  多线程构建,缓存loader  构建体积webpack-bundle-analyzer
        压缩包大小优化: gzip,静态资源压缩插件
        性能优化: 模块懒加载chunk-split   tree-sharking优化
```
...
请求头(referer,origin,cookie,connection,accept,authorizaiton,cache-control,accept-encoding,pragma,
...
toString()  from() filter() find() findIndex() every() some() reduceRight() reduce() map() foreach(function(currentValue,index,arr)) at(-1)
...
global:Number() String(obj)  isNaN() eval() Infinity NaN undefined 	isFinite() parseInt() parseFloat()
