...
请求头(referer,origin,cookie,connection,accept,authorizaiton,cache-control,
...
dom对象
只读属性
paddding+content：clientHeight clientWidth
...
#### display:none,visibility:hidden,opcatity:0区别
...
(4)display:inline-block/inline-grid/inline-table/table-caption/table-cellflex/table/grid
```
...
map() foreach(function(currentValue,index,arr))无返回值,速度快,可以改变原数组 at(-1)
...
数字方法：toPrecision()返回位数 toFixed()返回小数点后指定位数 toString(num) parseInt() parseFloat()

global：Number() String(obj)  isNaN() Number() eval() Infinity NaN undefined 	isFinite()
...
深拷贝：改变数据,不改变其他数据
实现:JSON.parse(JSON.stringify(obj,funciton,添加缩进))  Date,undefiend,Regexp,function,symbol不能正确拷贝

stringify(obj,(key,value)=>{console.log(`键名${key} 键值${value}`)} || ['要转换的对象'])
parse(obj,(key,value)=>{if(key==='') return value else return value+'拼接值'})
...
2.相同patchVnode：
```
双指针比对节点:头节点相同向后移;尾节点相同,向前移;头尾节点相同,头尾节点移动;都不相同查找旧节点是否存在新节点的key(是否可复用),存在复用,不存在新增;循环(对比)结束if(newNode>oldNode)创建new剩余未加入到真实dom中,else裁剪old剩余节点
```
...
3.同层级节点：
```
先遍历new节点,找到new在old的位置,记录上一节点的位置与当前遍历节点比较,if(preIndex<curIndex)不需要移动,否则需要移动位置，直接操作old将此时的节点移到已遍历节点的最后
定义标记变量find=false,在old中找到和new相同的key,find=true,遍历完old还为flase,新增节点
移动,插入操作完成后(两层遍历后),遍历old,找出new节点中不存在的并移除
此时old即为更新后的节点，直接渲染成真实dom
```
