...
JS对象常用方法
```
数组：fill[) slice[) splice[第a个元素前,删除b个,添加元素]
join pop push reverse sort unshift shift concat indexOf() lastIndexOf() includes()
filter find map foreach every some isArray  slice splice tostring

字符串:split  match replace匹配一个或多个值默认匹配一次g  search匹配单个下标 trim()去除两边空字符  substr[获得指定下标a，获取字符b个] substring[) startWith()指定字符开始  charAt()返回指定位置字符
