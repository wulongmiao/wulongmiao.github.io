...
baseURI domain  URL title cookie() lastModified最后一次修改u
...
removeAttribute() removeAttributeNode() removeChild() replaceChild() setAttribute() setAttributeNode()	textContent
...
a.isEqualNode(b0)ab两个元素相等 a.isSameNode(b)ab相同节点
nextSibling返回下一个同级节点 nextElementSibling只返回元素节点 
previousSibling前一个同级节点 previousElementSibling只返回元素节点 

clientHeight clientWidth元素的宽高(只包括padding) offsetHeight offsetWidth元素的宽高(包括滚动条，padding，border) offsetTop offsetLeft与父元素顶部，左边的距离  scrollLeft scrollTop顶部卷去的距离 scrollWidth  scrollHeight元素包括被卷去的总宽高
```
