...
Object:create(a,b)只能获得a本身的属性，不能读取a的原型 defineProperties(a,b)a对象上定义新的属性，或者修改现有属性
Object.defineProperty(obj,name,descriptor)  hasOwnProperty()是否具有指定的属性 A.isPrototypeOf(B)B是否存在于A的原型链上	 
freeze()对象永远是不可变  isFrozen() preventExtensions()可修改，删除现有属性，不能添加新属性 getOwnPropertyNames()指定对象的所有自身属性的属性名组成的数组
```
