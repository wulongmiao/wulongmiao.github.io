...
```
node --watch test.js 文件或依赖文件变化自动重启进程，可通过环境变量 NODE_WATCH 控制是否启用
node --experimental-* test.js 启用实验性特性
    --experimental-import-meta-resolve  启用 import.meta.resolve() 方法
    --experimental-permission  启用 权限模型
    --experimental-wasm-modules  允许将 WebAssembly 模块作为 ES 模块导入
    --experimental-global-webcrypto 在全局作用域暴露 crypto 对象

node --env-file=.dev.env --env-file=.env test.js 从文件加载环境变量
node --NODE_OPTIONS test.js 环境变量传递选项
node --test test.js 运行node内置运行器写的测试文件
node --eval test.js 直接执行代码
```
