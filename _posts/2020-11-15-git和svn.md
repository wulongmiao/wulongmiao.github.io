...
版本回退 git reset --hard HEAD^ 回退到上个版本
        ...
        git reset --hard 版本号
        git reset --soft 软回滚(回滚版本之后的提交记录清除，一般用于相同功能的提交记录规范)
        git revert 版本号 (回滚自己的提交，只撤销回滚的版本号，之后的提交记录会保存)
...
创建分支git checkout(切换到这个分支)  -b(创建) name tagName
...
git标签
git tag -a <tagName> -m '提交额外信息' //创建本地tag
git push origin <tagName> //推送到远程仓库
git push --tags // 推送本地所有标签
git tag -d <tagName> // 本地分支标签删除
git push origin :refs/tags/<tagName> // 远程库标签删除
```
